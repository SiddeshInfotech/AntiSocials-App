...
export default function FearTaskScreen() {
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(breathAnim, { toValue: 1.03, duration: 9000, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.timing(breathAnim, { toValue: 1, duration: 9000, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
      ])
    ).start();
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(bgShiftAnim, { toValue: 1, duration: 20000, easing: Easing.inOut(Easing.linear), useNativeDriver: true }),
        Animated.timing(bgShiftAnim, { toValue: 0, duration: 20000, easing: Easing.inOut(Easing.linear), useNativeDriver: true }),
      ])
    ).start();

    // Mascot animations (gently pulsing 👁️)
    Animated.loop(
      Animated.sequence([
        Animated.timing(mascotFloatAnim, { toValue: -6, duration: 3200, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
        Animated.timing(mascotFloatAnim, { toValue: 0, duration: 3200, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
      ])
    ).start();

    Animated.loop(
      Animated.sequence([
        Animated.timing(mascotScaleAnim, { toValue: 1.06, duration: 3200, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.timing(mascotScaleAnim, { toValue: 0.94, duration: 3200, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
      ])
    ).start();
    ...
    try {
      ...
      if (token) {
        const response = await apiFetch('/api/tasks/complete', {
          ...
          body: JSON.stringify({ task_name: 'Observe Fear Response' })
        });
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[StyleSheet.absoluteFillObject, { transform: [{ scale: breathAnim }] }]}>
        <Image 
          source={require('../assets/images/fear-bg.png')}
      ...
      <LinearGradient
        colors={['rgba(10, 15, 28, 0.4)', 'rgba(20, 25, 45, 0.65)', 'rgba(5, 8, 18, 0.95)']}
        style={StyleSheet.absoluteFillObject}
      />
      ...
      <SafeAreaView style={styles.foregroundLayer} edges={['top', 'bottom']}>
        ...
        >
          {!isCompleted && (
            <View style={styles.timerContentWrapper}>
              ...
              <Text style={styles.focusSubtitle}>
                {isPaused ? "Timer Paused" : "You're exploring your emotions..."}
              </Text>
          ...
          {isCompleted && (
            <View style={styles.successContainer}>
              ...
              <Text style={styles.successText}>You understood fear.</Text>
              <Text style={styles.successMessage}>Awareness reduces the power fear has over your decisions.</Text>
              <TouchableOpacity 
        ...
        >
          ...
          <View style={styles.glassPanel}>
            <Text style={styles.title}>Observe Fear Response</Text>
            ...
            <View style={styles.badgesRow}>
              ...
              <View style={[styles.badge, styles.badgePoints]}>
                <Feather name="award" size={14} color="#a5b4fc" />
            ...
            <View style={styles.descriptionList}>
              <Text style={styles.introText}>Fear is a natural emotion. Instead of reacting immediately, learn to observe it for five minutes:</Text>
              
              <View style={styles.bulletContainer}>
                <Text style={styles.descLine}>• Think about something that makes you uncomfortable</Text>
                <Text style={styles.descLine}>• Notice how your body responds & observe your breathing</Text>
                <Text style={styles.descLine}>• Watch your thoughts without judging or suppressing them</Text>
              </View>

              <Text style={styles.courageText}>"Fear loses its power the moment you become curious about it."</Text>
            </View>
