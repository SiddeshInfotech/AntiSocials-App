      ...
      <View style={styles.bottomArea}>
        <Animated.View style={{ transform: [{ scale: btnScale }] }}>
          <TouchableOpacity
            ...
            onPress={async () => {
              if (imageUri) {
                let pointsData = { pointsAdded: '1200', totalPoints: '0', streak: '0' };
                try {
                  ...
                  if (token) {
                    const response = await apiFetch('/api/tasks/complete', {
                      ...
                      body: JSON.stringify({ task_name: 'Plan one day group trip' })
                    });
                    const data = await response.json();
                    if (response.ok || data.success) {
                      pointsData = { 
                        pointsAdded: data.pointsAdded?.toString() || "1200", 
                  ...
                  console.error("Group trip complete API error:", e);
                }
